    ...
    const columns: Column<OnboardedBusiness>[] = [
        {
            ...
            render: (val, row) => (
                <div className="flex flex-col">
                    <span className="font-bold text-sm dark:text-brand-cream">{String(val)}</span>
        ...
        },
        {
            key: "phone",
            header: "Contact",
            render: (val) => <span className="text-xs font-mono dark:text-brand-cream">{String(val)}</span>
        },
        {
            key: "onboardedAt",
            header: "Date Added",
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-serif font-medium tracking-tight text-brand-deep dark:text-brand-cream">My Businesses</h1>
            ...
            <div className="grid grid-cols-1 gap-5 md:hidden">
                {paginatedBusinesses.map((biz) => (
                    <GlassCard
                        key={biz.id}
                        className="p-5 rounded-3xl before:rounded-3xl border border-brand-deep/5 bg-white/40 dark:bg-white/5 active:scale-[0.98] transition-all relative overflow-hidden group"
                    ...
                    >
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5">
                                        ...
                                        <span className="w-1 h-1 rounded-full bg-brand-deep/10 dark:bg-brand-cream/10" />
                                        <span className="text-[10px] text-brand-deep/40 dark:text-brand-cream/40 font-bold uppercase tracking-wider">{biz.ownerName}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex items-end justify-between pt-4 border-t border-brand-deep/5 dark:border-white/5">
                            ...
                            <div className="text-right">
                                <p className="text-[8px] font-black text-brand-deep/30 dark:text-brand-cream/30 uppercase tracking-[0.2em] mb-1">Total Earned</p>
            ...
            {/* Desktop Table View */}
            <GlassCard className="hidden md:block overflow-hidden border-none shadow-2xl shadow-brand-deep/5 dark:shadow-none">
                <DataTable
                    columns={columns}
            ...
            {totalPages > 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-4 pb-8">
                    <p className="text-xs font-bold text-brand-deep/40 dark:text-brand-cream/40 uppercase tracking-widest leading-none">
