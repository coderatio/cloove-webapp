...
export interface BusinessConfigs {
  ...
  hotel_deposit_enabled?: boolean;
  hotel_deposit_percent?: number;
  hotel_negotiation_enabled?: boolean;
...
function stripQuietFlag(
  variables: Partial<BusinessConfigs> & { quiet?: boolean },
): Partial<BusinessConfigs> {
  return Object.fromEntries(
    Object.entries(variables).filter(([key]) => key !== "quiet"),
  ) as Partial<BusinessConfigs>;
}
...
export const useUpdateBusinessSettings = () => {
  ...
  return useMutation({
    mutationFn: (variables: Partial<BusinessConfigs> & { quiet?: boolean }) => {
      ...
      return apiClient.patch<MutationResponse>("/settings/business", {
        configs,
      });
    },
...
export const useUpdateProfile = () => {
  ...
  return useMutation({
    mutationFn: (payload: UpdateProfilePayload) =>
      apiClient.patch<MutationResponse>("/settings/profile", payload, {
        fullResponse: true,
      }),
    onSuccess: (data) => {
